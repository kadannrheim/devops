...
# Сборка
# Перезапускаем сборку с очисткой кэша
docker build --no-cache -t golang-redis-app:0.0.2 -f Dockerfile.txt .
# Обычная пересборка
docker build -t kadannr/golang-redis-app:0.0.2 -f Dockerfile.txt .
