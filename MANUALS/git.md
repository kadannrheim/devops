...
`git config --global user.name "kadannr"`
...
`ssh-keygen -t rsa -b 4096 -C "kadannr@gmail.com"`
2. ssh-agent — программа для хранения и управления SSH-ключами. Давайте запустим её и добавим туда наш SSH-ключ. Если в ответ терминал покажет надпись «Agent pid» и число — значит, всё ок, агент запущен.
`eval "$(ssh-agent -s)"`
3. Теперь добавим наш ключ командой. Вводим пароль и если видет то всё ок Identity added
`ssh-add ~/.ssh/github`
4. Добавляем ключ на github. Катнем публичный и скопируем его
`cat ~/.ssh/github.pub`
Идём на github/Settings/SSH and GPG keys/New SSH key и добавляем публичный ключ
5. Если нужно сделать форк общего репозитория, то вот инструкция https://htmlacademy.ru/blog/git/git-console
6. Копируем репозиторий к себе
`git clone git@github.com:kadannrheim/ansible.git`
Результат появится папка как и имя созданного репозитрия в гите:
`ls`
вывод`ansible`
7. В каждом репозитории есть как минимум одна основная ветка, которую создаёт сам Git — она называется master. Создадм второстепенную для правок. Отобразить список веток
`git branch`
Если мы не в мастере, то переключимся на нее
`git checkout master`
Создаём ветку
`git checkout -b pre`
Смена названия ветки
`git branch -m старое-имя-ветки новое-имя-ветки`
8. До коммитов добавлям информацию о себе
`git config --global user.name "kadannr"`
`git config --global user.email "kadannr@gmail.com"`
`git config --global core.editor vim` и редактор
9. Сохраняем изменения все
`git add -A`
Выборочно
`git add web-hotls.yml`
Отмена
`git reset`
10. Делаем коммит (Сделать коммит — значит зафиксировать все сохранённые изменения и дать им название). Текст сообщения должен быть лаконичным и вместе с этим сообщать о том, что делает коммит (внесённые изменения)
`git commit -m "adding ansible files"`
11. Отправляем изменения на GitHub. Сохранённые изменения пока не видны коллегам, потому что находятся в нашем локальном репозитории. Нужно отправить коммиты на GitHub. Где origin означает репозиторий на компьютере, то есть ваш форк. Слово origin — часть команды, не меняйте это название на своё. Указываем имя текущей ветки.
`git push origin main`
12. Через UI согласуем Compare & pull request если ветка не основная если это форк

# Ветки
Инфо по веткам
`git branch` 
Посмотреть последнии комиты по веткам
`git branch -v`
Опции —merged и —no-merged, с помощью которых можно отфильтровать полученную последовательность веток. То есть мы получим список ответвлений, которые уже были слиты, или, наоборот, ветки, которые еще не прошли через слияние с другими
`git branch --merged`
>* main
`git branch --no-merged`
>pre

# Отмена локальных изменнеий
git reset --hard HEAD  # Сбросить все локальные изменения
git clean -fd         # Удалить неотслеживаемые файлы, не обязательно
git pull origin main  # Получить последние изменения из удалённого репозитория
